...
4. **触发器**：每天，时间设为 **8:00**
5. **操作**：启动程序
   - **程序或脚本**：`uv` 的完整路径（用 `where.exe uv` 查询，如 `C:\Users\你\.local\bin\uv.exe`）
   - **添加参数**：`run python daily_digest.py`
   - **起始于**：项目根目录（如 `C:\path\to\auto-cnki`）
6. 如需开机即运行，任务属性中勾选「**不管用户是否登录都要运行**」

> 💡 macOS / Linux 用户可用 `cron` 或 `launchd` 实现定时。
